...
class PPOLearner():
    ...
    def gather_rollouts(self) -> Dict[str, Any]:
        ...
        while any(active_list) and episode_step < self.max_steps_per_episode:
            ...
            # FIXED: create actions dict correctly - map agent indices to actions
            active_indices = [i for i, is_active in enumerate(active_list) if is_active]
            actions_dict = {agent_idx: int(actions[i].item()) for i, agent_idx in enumerate(active_indices)}
            ...
            # FIXED: extract rewards and dones for active agents correctly
            rewards_list = [rewards[agent_idx] for agent_idx in active_indices]
            dones_list = [dones[agent_idx] for agent_idx in active_indices]
            ...
            self.rollout.add_transitions(states=current_state_tensor[active_list].detach(),
            ...
            # FIXED: update active_list and current_state_tensor correctly
            for i, agent_idx in enumerate(active_indices):
                active_list[agent_idx] = not dones[agent_idx]
            
            # Update current_state_tensor to next_state for ALL agents (not just active)
            current_state_tensor = obs_dict_to_tensor(observation=next_state_dict, 
                                                      obs_type=self.obs_type, 
                                                      n_agents=self.n_agents,
                                                      max_depth=self.max_depth,
                                                      n_nodes=self.controller.config['n_nodes'])
    ...
    def _loss(self, minibatch: Dict[str, Tensor]) -> Tuple[Tensor, Tensor, Tensor]:
        """
        Calculate actor/critic loss with correct entropy sign and shape-safe value targets.
        """
        # ------------------
        # policy (actor)
        # ------------------
        ...
        if self.importance_sampling:
            critic_loss = value_loss_with_IS(
                predicted_values=minibatch['new_state_values'].squeeze(-1),
                bootstrap_values=minibatch['bootstrap_values'].squeeze(-1),
                new_state_values=minibatch['new_state_values'].squeeze(-1),
                new_log_prob=minibatch['new_log_probs'],
                old_log_prob=minibatch['log_probs'],
            ...
            # FIXED: Use actual returns, not state_values + gaes
            value_targets = minibatch['returns'].detach()
            predicted_values = minibatch['new_state_values'].squeeze(-1)
        ...
        # entropy bonus
        # ------------------
        entropy = minibatch['entropy'].mean()
    ...
    def _gaes(self) -> Tuple[float, float, float, float]:
        """
        Compute GAE for all episodes and agents in the rollout buffer.
        Also computes returns for value function training.
        Returns both raw and normalised GAE statistics.
        """
        for idx, episode in enumerate(self.rollout.episodes):
            self.rollout.episodes[idx]['gaes'] = [[] for _ in range(self.n_agents)]
            self.rollout.episodes[idx]['returns'] = [[] for _ in range(self.n_agents)]  # ADDED
            
            for agent in range(len(episode['states'])):
                state_values = torch.stack(episode['state_values'][agent]).detach().squeeze(-1)
                next_state_values = torch.stack(episode['next_state_values'][agent]).detach().squeeze(-1)

                rewards = torch.tensor(episode['rewards'][agent], dtype=torch.float32)
                dones = torch.tensor(episode['dones'][agent], dtype=torch.float32)

                gaes = torch.zeros_like(rewards)
                returns = torch.zeros_like(rewards)  # ADDED
                advantage = 0.0
                for t in reversed(range(len(rewards))):
                    ...
                    # ADDED: Compute the actual return (TD target for value function)
                    returns[t] = rewards[t] + self.gamma * next_state_values[t] * (1 - dones[t])

                self.rollout.episodes[idx]['gaes'][agent] = gaes
                self.rollout.episodes[idx]['returns'][agent] = returns  # ADDED
    ...
    def _normalise_gaes(self) -> Tuple[float, float, float, float]:
        """
        Normalise all GAEs across episodes and agents.
        Returns: raw_mean, raw_std, normalised_mean, normalised_std
        """
        n_episodes = len(self.rollout.episodes)

        # Stack all GAEs safely
        all_gaes = []
        for episode in self.rollout.episodes:
            for agent_gae in episode['gaes']:
                all_gaes.append(agent_gae)
        stacked_gaes = torch.cat(all_gaes, dim=0)
        ...
        for idx in range(n_episodes):
            for agent in range(self.n_agents):
                self.rollout.episodes[idx]['gaes'][agent] = (self.rollout.episodes[idx]['gaes'][agent] - raw_gae_mean) / raw_gae_std

        # Compute stats for logging
        norm_all_gaes = torch.cat([gae for episode in self.rollout.episodes for gae in episode['gaes']], dim=0)
        gae_mean = norm_all_gaes.mean().item()
        gae_std = norm_all_gaes.std().item()
