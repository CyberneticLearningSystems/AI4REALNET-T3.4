...
class MultiAgentRolloutBuffer:
    ...
    def _reset_current_episode(self) -> None:
        self.current_episode: Dict[str, List] = {
            ...
            'returns': [[] for _ in range(self.n_agents)],  # ADDED
    ...
    def get_transitions(self, gae: bool = True) -> None:
        ...
        for episode in self.episodes:
            for agent_handle in range(self.n_agents):
                ...
                if gae and len(episode['gaes'][agent_handle]) > 0:
                    # gaes[agent_handle] is already a tensor, so append it to the list
                    gaes.append(episode['gaes'][agent_handle])
                    returns.append(episode['returns'][agent_handle])
        ...
        if gae and len(gaes) > 0:
            # gaes is a list of tensors, concatenate them
            transitions_dict['gaes'] = torch.cat(gaes, dim=0).clone().detach()
            transitions_dict['returns'] = torch.cat(returns, dim=0).clone().detach()
